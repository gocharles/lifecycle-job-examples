...
echo "Fetching latest file from S3: ${SEED_FOLDER}"
latest_file=$(aws s3 ls "${SEED_FOLDER}" | sort -k1,2 | tail -n 1 | awk '{print $4}')

if [[ -z "${latest_file}" ]]; then
  echo "No files found in S3 path: ${SEED_FOLDER}"
  exit 1
fi

# Download the latest file from S3
echo "Downloading file: ${latest_file}"
aws s3 cp "${SEED_FOLDER}/${latest_file}" seed.sql
